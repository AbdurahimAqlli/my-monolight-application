...
/**
 * REST controller for managing {@link uz.asz.myapp.domain.Student}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class StudentResource {
    ...
    public List<Student> getAllStudents(@RequestParam(required = false) String filter) {
        if ("car-is-null".equals(filter)) {
            log.debug("REST request to get all Students where car is null");
            return StreamSupport
                .stream(studentRepository.findAll().spliterator(), false)
                .filter(student -> student.getCar() == null)
